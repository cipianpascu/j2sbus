...
import com.ciprianpascu.j2sbus.modbus.io.ModbusSerialTransaction;
import com.ciprianpascu.j2sbus.modbus.io.ModbusSerialTransport;
import com.ciprianpascu.j2sbus.modbus.io.ModbusTransaction;
import com.ciprianpascu.j2sbus.modbus.msg.*;
import com.ciprianpascu.j2sbus.modbus.net.ModbusMasterFactory;
...
public class ReadCommEventLogTest {
    ...
    private static final Logger logger = LoggerFactory.getLogger(ReadCommEventLogTest.class);

    private static void printUsage() {
        System.out.printf("\nUsage:\n    java com.ciprianpascu.j2sbus.modbus.cmd.ReadCommEventLogTest <address{:port} [String]> <unit [int]> {<repeat [int]>}");
    }
        ...
        try {
            ...
            for (int k = 0; k < repeat; k++) {
                ...
                if (res != null) {
                    ...
                    System.out.printf("No response to GET COMM EVENT LOG request");
                }
                ...
                System.out.printf("Status: %d, Events: %d, Messages: %d, Entries: %d", data.getStatus(), data.getEventCount(), data.getMessageCount(), data.getEvents().length);
                System.out.printf("Entries: %s", Arrays.toString(data.getEvents()));
            }
        }
        catch (Exception ex) {
            ex.printStackTrace();
